...
<body id="page-top">
    ...
    <section class="page-section bg-light" id="portfolio">
        <div class="container">
            <div class="text-center">
                <h2 class="section-heading text-uppercase">UNIT</h2>
            </div>
            <div class="row">
                @foreach ($c as $ca)
                <div class="col-lg-4 col-sm-6 mb-4">
                    ...
                    <div class="portfolio-item">
                        <a class="portfolio-link" data-bs-toggle="modal" href="#portfolioModal{{ $ca->id }}">
                            ...
                            <img class="img-fluid portfolio-img-square" src="{{ asset($ca->file) }}" alt="Unit Image" />
                        </a>
                        <div class="portfolio-caption">
                            <div class="portfolio-caption-heading">{{ $ca->unit }}</div>
                            <div class="portfolio-caption-subheading text-muted">{{ $ca->status }}</div>
                        </div>
                    </div>
                </div>
                @endforeach
    ...
    <!-- Portfolio Modals-->
    @foreach ($c as $ca)
    <div class="modal fade" id="portfolioModal{{ $ca->id }}" tabindex="-1" aria-labelledby="portfolioModalLabel{{ $ca->id }}" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title text-uppercase" id="portfolioModalLabel{{ $ca->id }}">{{ $ca->unit }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body text-center">
                    @if ($ca->interior_image)
                        <img class="img-fluid d-block mx-auto mb-4" src="{{ asset($ca->interior_image) }}" alt="Interior Image" />
                    @else
                        <p class="text-muted">No interior image available</p>
                    @endif
                    <p class="item-intro text-muted">{{ $ca->feature ?? 'No description available' }}</p>
                </div>
